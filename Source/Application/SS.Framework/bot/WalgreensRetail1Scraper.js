...
var core = async (configuration, onsuccess, onfailure) => {
    try {
        ...
        var companyElement = await driver.findElement(By.xpath('//*[@id="advancedSearchInterface.jobfield1L1"]/option[2]'));
        await companyElement.click();
        await driver.wait(until.elementLocated(By.xpath('//*[@id="advancedSearchInterface.jobfield1L2"]')), 10000);
        var categoryElement = await driver.findElement(By.xpath('//select[@id="advancedSearchInterface.jobfield1L2"]'));
        var optionArray = await categoryElement.findElements(By.tagName('option'));

        for (var i = 2; i <= optionArray.length; i++) {
            var option = await driver.findElement(By.xpath('//select[@id="advancedSearchInterface.jobfield1L2"]/Option[' + i + ']'));
            ...
            if (category == 'Retail') {
                ...
                do {
                    ...
                    try {
                        var HomeElement = await driver.findElements(By.xpath('//*[@id="requisitionListInterface.pagerDivID5126.panel.Next"]/span[@class="pagerlink"]'));
                        var home = await HomeElement.length;
                        if (home) {
                            var nextLink = await driver.findElement(By.xpath('//*[@id="requisitionListInterface.pagerDivID5126.Next"]'));
