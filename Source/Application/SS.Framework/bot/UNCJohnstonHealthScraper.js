...
var core = async (configuration, onsuccess, onfailure) => {
    ...
    var jobs = new Array();
    try {
        await driver.get('https://www.healthcaresource.com/johnstonmem/index.cfm?fuseaction=search.categoryList&template=dsp_job_categories.cfm');
        ...
        for (var i = 0; i < optionArray.length - 1; i++) {
            ...
            var pagination = true;
            do {
                var tagElementxpath = '//*[@id="searchResultsForm"]/fieldset/div[4]/div/div/div';
                var isPresent = true;
                do {
                    try {
                        var job = jobMaker.create();
                        var jobElements = await driver.findElements(By.xpath(tagElementxpath + "/div[1]/div/div/div"));
                        if (jobElements.length < 1)
                            isPresent = false;
                        if (isPresent) {
                            var locTagElementXpath1 = '/div[1]/div/div/div/div[2]/div/p';
                            ...
                            if (location) {
                                ...
                                if (locationValue.length == 2) {
                                    job.JOB_LOCATION_CITY = locationValue[1];
                                }
                            }

                            job.JOB_CATEGORY = optionValue;
                            var titleElement = await driver.findElement(By.xpath(tagElementxpath + '/div[1]/div/div/div/div[1]/div/h4/strong/a'));
                            ...
                            if (applyURL) {
                                ...
                                job.JOB_APPLY_URL = "https://www.healthcaresource.com/johnstonmem/index.cfm?fuseaction=apply.login&template=dsp_apply_login.cfm&cJobId=" + parseInt(jobId[0]);
                            }
                    ...
                    tagElementxpath = tagElementxpath + '/div[2]/div/div/div';
