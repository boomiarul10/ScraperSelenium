...
var core = async (configuration, onsuccess, onfailure) => {
    try {
        ...
        await driver.get('https://citi.taleo.net/careersection/2/moresearch.ftl?lang=pl');
        ...
        var record = atscount.split("stanowisk:").pop().split(")").shift();
                    ...
                    if (isPresent) {
                        try {
                            ...
                            if (job.JDTID_UNIQUE_NUMBER) {
                                var url = "https://citi.taleo.net/careersection/2/jobdetail.ftl?lang=pl&job=" + job.JDTID_UNIQUE_NUMBER;
                            }
                            ...
                            if (isDetailPage) {
                                ...
                                if (location) {
                                    ...
                                    if (location.includes('-')) {
                                        ...
                                        if (job.JOB_LOCATION_COUNTRY) {
                                            if (job.JOB_LOCATION_COUNTRY == "POL") {
                                                job.JOB_LOCATION_COUNTRY = "Polska";
                                            }
                                        }
                                    }
                                }

                                var typeData = await driverjobdetails.findElements(By.xpath("//div[@id='requisitionDescriptionInterface.ID1905.row1' and //*[contains(text(), 'Harmonogram')]]//span[3]"));
                                var isTypePresent = await typeData.length;
                                if (isTypePresent) {
                                    var typeElement = await driverjobdetails.findElement(By.xpath("//div[@id='requisitionDescriptionInterface.ID1905.row1' and //*[contains(text(), 'Harmonogram')]]//span[3]"));
                                ...
                                var qualificationData = await driverjobdetails.findElements(By.xpath("//div[@id='requisitionDescriptionInterface.ID1855.row1' and //*[contains(text(), 'Stanowisko')]]//span[3]"));
                                var isQualificationPresent = await qualificationData.length;
                                if (isQualificationPresent) {
                                    var qualificationElement = await driverjobdetails.findElement(By.xpath("//div[@id='requisitionDescriptionInterface.ID1855.row1' and //*[contains(text(), 'Stanowisko')]]//span[3]"));
                                ...
                                job.JOB_APPLY_URL = "https://citi.taleo.net/careersection/2/jobdetail.ftl?lang=pl&job=" + job.JDTID_UNIQUE_NUMBER;
