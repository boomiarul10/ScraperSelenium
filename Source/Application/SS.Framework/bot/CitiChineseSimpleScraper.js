...
var core = async (configuration, onsuccess, onfailure) => {
    try {
        ...
        await driver.get('https://citi.taleo.net/careersection/2/jobdetail.ftl?lang=zh-CN');
                ...
                do {
                    ...
                    if (isPresent) {
                        try {
                            ...
                            var locationElement = await driver.findElement(By.xpath('//*[@id="requisitionListInterface.listRequisition"]/tbody/tr[@class="ftlcopy ftlrow"][' + counter + ']//span[contains(@id,"requisitionListInterface.reqBasicLocation")]'));
                            var location = await locationElement.getText();

                            var url = "https://citi.taleo.net/careersection/2/jobdetail.ftl?lang=zh-CN&job=" + job.JDTID_UNIQUE_NUMBER;
                            ...
                            desc = desc.split('<span class="subtitle">说明</span>');
                            desc = '<span class="subtitle">说明</span>' + desc[1];
                            ...
                            if (location) {
                                location = location.replace('亚太地区, APAC-CHN', '中国').replace('APAC-中国', '中国').replace('LATAM-', '').replace('CHL', 'Chile');
                                ...
                                if (loc.length == 3) {
                                    ...
                                    country = country.replace('NAM-USA', 'USA').replace('APAC-CHN', '中国');
