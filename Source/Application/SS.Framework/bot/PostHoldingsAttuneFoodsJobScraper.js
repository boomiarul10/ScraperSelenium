...
var core = async (configuration, onsuccess, onfailure) => {
    try {
        ...
        await driver.get('https://recruiting2.ultipro.com/POS1003POST/JobBoard/faf64f8d-5979-4c95-b18c-73cf7e57ec54');
        ...
        var categoryElement = await driver.findElement(By.xpath("//select[@data-automation='department-dropdown']"));
        var optionArray = await categoryElement.findElements(By.tagName('option'));

        for (var i = 2; i <= optionArray.length; i++) {
            var option = await driver.findElement(By.xpath("//select[@data-automation='department-dropdown']/Option[" + i + "]"));
                ...
                if (isPresent) {
                    try {
                        ...
                        if (url) {
                            ...
                            var idRex = /.*opportunityId...(.*)/;
                            var idRexPresent = idRex.test(url);

                            if (idRexPresent) {
                                var idData1 = idRex.exec(url);
                                job.JDTID_UNIQUE_NUMBER = idData1[1];
                            } 
                            idRex.lastIndex = 0;                            
                        }
            ...
            var reset = await driver.findElement(By.xpath("//a[@data-automation='reset-button']")).click();
            await driver.sleep(1000);
        }
