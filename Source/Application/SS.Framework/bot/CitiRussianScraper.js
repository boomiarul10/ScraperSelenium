...
var core = async (configuration, onsuccess, onfailure) => {
    try {
        ...
        await driver.get('https://citi.taleo.net/careersection/2/jobdetail.ftl?lang=ru');
        await driver.sleep(5000);
        var searchLink = await driver.findElement(By.xpath('//*[@id="topNavInterface.advancedSearchTabAction"]'));
        await searchLink.click();

        var jobCountElement = await driver.findElement(By.xpath('//*[@id="requisitionListInterface.searchResultHeaderId"]//span[contains(text(),"вакансий")]'));
        var atsCount = await jobCountElement.getText();
        var jobCount = atsCount.split("найдено");
        var atsJobCount = jobCount[1].split("вакансий");
            ...
            do {
                ...
                if (isPresent) {
                    try {
                        ...
                        var locationElement = await driver.findElement(By.xpath('//*[@id="requisitionListInterface.listRequisition"]/tbody/tr[' + counter + ']//span[contains(@id,"requisitionListInterface.reqBasicLocation")]'));
                        var location = await locationElement.getText();
                        job.JOB_APPLY_URL = "https://citi.taleo.net/careersection/2/jobdetail.ftl?lang=ru&job=" + job.JDTID_UNIQUE_NUMBER;

                        var jobDetailURL = "https://citi.taleo.net/careersection/2/jobdetail.ftl?lang=ru&job=" + job.JDTID_UNIQUE_NUMBER;
                        ...
                        if (jobDetail.length == 1) {
                            ...
                            var descr = desc1.split('<span id="requisitionDescriptionInterface.ID1578.row1" class="text" title="">');
                            var desc = descr[1].split('<div id="requisitionDescriptionInterface.ID1755');
                            var description = 'Описание' + desc[0].replace('<h2 xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:ftl="http://www.taleo.net/ftl" xmlns:htm="http://www.w3.org/1999/xhtml" class="no-change-header-inline"><div id="requisitionDescription<br>Interface.d26612e324.row1" class="inlinepanel" title="" style="DISPLAY: inline"> Qualificações</div></h2>', '')
                            ...
                            if (location != null) {
                                ...
                                location = location.replace("Азия, APAC-CHN", "China").replace("IND", "IN").replace(" (Nanking)", "").
                                    replace("CHL", "Chile").replace("LATAM-", "").replace("RUS", "Russia").
                                    replace("SPE", "Russia").replace("Saint", "SPE").replace("Petersburg", "Saint Petersburg");
