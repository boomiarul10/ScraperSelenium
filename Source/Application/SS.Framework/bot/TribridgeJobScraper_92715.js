...
var core = async (configuration, onsuccess, onfailure) => {
    try {
        ...
        await driver.get('https://chj.tbe.taleo.net/chj05/ats/careers/jobSearch.jsp?cws=1&org=TRM');
        var loop;
        ...
        var recordCount = record[1].split("open job(s)").shift().trim();
        jobMaker.setatsJobCount(parseInt(recordCount));
                ...
                do {
                    ...
                    if (isPresent) {
                        try {
                            ...
                            var departmentElement = await driverjobdetails.findElement(By.xpath("//table[@role='presentation']//tr[td[contains(text(), 'Department:')]]/td[2]/b"));
                            var department = await departmentElement.getText();

                            if (department) {
                                job.JOB_CATEGORY = ((job.JOB_CATEGORY.length) > 0) ? job.JOB_CATEGORY : department;
                            }
        ...
        await driver.get('https://chj.tbe.taleo.net/dispatcher/servlet/DispatcherServlet?org=TRM&act=redirectCws&cws=38');
        var loop;
        var jobCount = await driver.wait(until.elementLocated(By.xpath("//table[@role='presentation']/tbody//tr[td[contains(.,'open job(s)')]]")), 4000);
        var atsValue = await jobCount.getText();
        var record1 = atsValue.split("total of");
        var recordValue = record1[1].split("open job(s)").shift().trim();
        var value = parseInt(recordCount) + parseInt(recordValue);
        jobMaker.setatsJobCount(parseInt(value));

        var categoryElement = await driver.findElement(By.xpath("//select[@name='location']"));
        var optionArray = await categoryElement.findElements(By.tagName('option'));

        for (var j = 2; j <= optionArray.length; j++) {
            await driver.findElement(By.xpath("//select[@name='location']/Option[1]")).click();
            var option = await driver.findElement(By.xpath("//select[@name='location']/Option[" + j + "]"));
            var category = await option.getAttribute('text');
            await option.click();
            var submitElement = await driver.findElement(By.xpath("//input[@name='tbe_cws_submit']"));
            await submitElement.click();
            ...
            do {
                ...
                try {
                    ...
                    var firstCountData = await driver.findElement(By.xpath("//table[@role='presentation']/tbody//tr//td[contains(.,'of')]/b[1]"));
                    var firstCount = await firstCountData.getText();
                    var secndCountData = await driver.findElement(By.xpath("//table[@role='presentation']/tbody//tr//td[contains(.,'of')]/b[2]"));
                    var secndCount = await secndCountData.getText();
                    if (firstCount) {
                        firstCount = firstCount.split("-")[1].trim();
                        if (secndCount) {
                            secndCount = secndCount.trim();
                            if (firstCount != secndCount) {
                                var nextContainer = await driver.findElements(By.xpath("//table[@role='presentation']//input[@title='Next Page']"));
                                var next = nextContainer.length;
                                if (next) {
                                    var nextLink = await driver.findElement(By.xpath("//table[@role='presentation']//input[@title='Next Page'][1]"));
