...
var core = async (configuration, onsuccess, onfailure) => {
    try {
        ...
        await driver.get('https://jobs.lahey.org/psp/erecruit/EMPLOYEE/HRMS/c/HRS_HRAM.HRS_CE.GBL?Page=HRS_CE_HM_PRE&Action=A&SiteId=102');
                ...
                if (isPresent) {
                    try {
                        ...
                        while (run != "completed") {
                            ...
                            var url = "https://jobs.lahey.org/psp/erecruit/EMPLOYEE/HRMS/c/HRS_HRAM.HRS_CE.GBL?Page=HRS_CE_JOB_DTL&Action=A&JobOpeningId=" + job.JDTID_UNIQUE_NUMBER + "&SiteId=102&PostingSeq=1";
                            ...
                            try {
                                ...
                                if (isDetailPage) {
                                    ...
                                    var descElement = await driverjobdetails.findElement(By.xpath('//*[@id="win0divPSPAGECONTAINER"]//tr[14]'));
                                    
                                    var desc = await descElement.getAttribute("outerHTML");
                                    desc = desc.replace('<span class="PABOLDTEXT" id="HRS_SCH_PSTDSC_DESCR$1">Responsibilities</span>', '<br><b>Responsibilities</b><br><br>');
                                    desc = desc.replace('<span class="PABOLDTEXT" id="HRS_SCH_PSTDSC_DESCR$2">Qualifications</span>', '<br><b>Qualifications</b><br><br>');
                                    desc = desc.replace('<span class="PABOLDTEXT" id="HRS_SCH_PSTDSC_DESCR$3">Shift</span>', '<br><b>Shift</b><br><br>');
                                    desc = desc.replace('<span class="PABOLDTEXT" id="HRS_JO_DSC_TYP_DESCR$0">Job Description</span>', '<b>Job Description</b><br><br>');
                                    ...
                                    desc = desc.replace(/<tr/g, "<font");
                                    desc = desc.replace(/<\/tr/g, "</font");
                                    desc = desc.replace(/<td/g, "<font");
                                    desc = desc.replace(/<\/td/g, "</font");
                                    desc = desc.replace(/<th/g, "<font");
                                    desc = desc.replace(/<\/th/g, "</font");
                                    desc = desc.replace(/<tbody/g, "<font");
                                    desc = desc.replace(/<\/tbody/g, "</font");
                                   desc = desc.replace(/<span/g, "<font");
                                    desc = desc.replace(/<\/span/g, "</font");
                                    desc = desc.replace(/<div/g, "<font");
                                    desc = desc.replace(/<\/div/g, "</font");
                                    ...
                                    var statusElement = await driverjobdetails.findElements(By.xpath('//*[@id="win0divHRS_CE_WRK2_HRS_FULL_PART_TIME$0"]/span'));
